...
class ShareJSBSimRunner(Runner):
    ...
    def load(self):
        ...
        self.num_ai_agents = self.envs.num_ai_agents
        ...
        # buffer
        self.buffer = SharedReplayBuffer(self.all_args, self.num_ai_agents, self.obs_space, self.share_obs_space, self.act_space)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if (episode % self.save_interval == 0) or (episode == episodes - 1):
                self.save(episode, train_infos["average_episode_rewards"])

            # tensorbard print
            self.writer.add_scalar("Total Reward", train_infos["average_episode_rewards"], episode)
    ...
    def warmup(self):
        ...
        if self.use_selfplay:
            self.opponent_obs = obs[:, self.num_ai_agents:, ...]
            obs = obs[:, :self.num_ai_agents, ...]
            share_obs = share_obs[:, :self.num_ai_agents, ...]
        self.buffer.step = 0
    ...
    def insert(self, data: List[np.ndarray]):
        ...
        masks = np.ones((self.n_rollout_threads, self.num_ai_agents, 1), dtype=np.float32)
        masks[dones_env == True] = np.zeros(((dones_env == True).sum(), self.num_ai_agents, 1), dtype=np.float32)

        active_masks = np.ones((self.n_rollout_threads, self.num_ai_agents, 1), dtype=np.float32)
        active_masks[dones == True] = np.zeros(((dones == True).sum(), 1), dtype=np.float32)
        active_masks[dones_env == True] = np.ones(((dones_env == True).sum(), self.num_ai_agents, 1), dtype=np.float32)
    ...
    def save(self, episode, reward):
        ...
        torch.save(
            {
                'episode' : episode,
                'model_dict' : policy_actor_state_dict,
                'optimizer_dict' : self.policy.optimizer.state_dict
            },
            str(self.save_dir) + '/actor_' + str(reward) + "_" + str(episode) + ".pt")
        ...
        torch.save(
            ...
            str(self.save_dir) + '/actor_latest.pt')
        
        policy_critic_state_dict = self.policy.critic.state_dict()
        torch.save(
            {
                'episode' : episode,
                'model_dict' : policy_critic_state_dict,
                'optimizer_dict' : self.policy.optimizer.state_dict
            },
            str(self.save_dir) + '/critic_' + str(episode) + ".pt")
        ...
        torch.save(
            ...
            str(self.save_dir) + '/critic_latest.pt')
