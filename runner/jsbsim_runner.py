...
class JSBSimRunner(Runner):
    ...
    def insert(self, data: List[np.ndarray]):
        ...
        # dones : Rollout Thread / Agent / Done
    ...
    def restore(self):
        actor_check_pt = torch.load(str(self.model_dir) + '/actor_latest.pt')
        critic_check_pt = torch.load(str(self.model_dir) + '/critic_latest.pt')

        # actor_check_pt = torch.load(str(self.model_dir) + '/actor_2790.pt')
        # critic_check_pt = torch.load(str(self.model_dir) + '/critic_230826.15625_2790.pt')

        # print("LOAD SUCCESS")

        # self.policy.actor.load_state_dict(actor_check_pt)
        # self.policy.critic.load_state_dict(critic_check_pt)

        self.policy.actor.load_state_dict(actor_check_pt['model_dict'])
        self.policy.critic.load_state_dict(critic_check_pt['model_dict'])
        # self.policy.optimizer.load_state_dict(actor_check_pt['optimizer_dict'])