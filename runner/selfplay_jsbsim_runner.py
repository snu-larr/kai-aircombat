...
class SelfplayJSBSimRunner(JSBSimRunner):
    ...
    def insert(self, data: List[np.ndarray]):
        ...
        # dones shape : (# of rollout threads, # of Agents, 1)
        dones_env = np.all(dones.squeeze(axis=-1), axis=-1)

        # rnn_states_actor shape : (# of rollout threads, 1, 1, 128)
        ...
        # masks shape : (# of rollout threads, # of Agents, 1)
    ...
    def eval(self, total_num_steps):
        ...
        while total_episodes < self.eval_episodes:
            ...
            if total_episodes >= eval_cur_opponent_idx * eval_each_episodes:
                ...
                self.eval_opponent_policy.actor.load_state_dict(torch.load(str(self.save_dir) + f'/actor_{policy_idx}.pt')['model_dict'])
    ...
    def save(self, episode, reward):
        ...
        torch.save(
            ...
            str(self.save_dir) + '/actor_' + str(episode) + ".pt")
        ...
        torch.save(
            ...
            str(self.save_dir) + '/critic_' + str(reward) + "_" + str(episode) + ".pt")
    ...
    def reset_opponent(self):
        ...
        for policy in self.opponent_policy:
            ...
            policy.actor.load_state_dict(torch.load(str(self.save_dir) + f'/actor_{choose_idx}.pt')['model_dict'])
            policy.prep_rollout()
    ...
    def render(self):
        ...
        self.policy.actor.load_state_dict(torch.load(str(self.model_dir)+ f'/actor_{idx}.pt')['model_dict'])
        self.policy.prep_rollout()
        self.eval_opponent_policy.actor.load_state_dict(torch.load(str(self.model_dir) + f'/actor_{opponent_idx}.pt')['model_dict'])
        self.eval_opponent_policy.prep_rollout()
        logging.info("\nStart render ...")
